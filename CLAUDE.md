...
Both live at the repo root and automate everything in the "Running locally" section below — idempotent, safe to re-run (they skip anything already done and never overwrite an existing `.env`). `setup.bat` needs Docker Desktop, Python (via the `py` launcher, whatever version it defaults to — no version gate; if the venv creation later fails, see "Backend venv" below for the one known-bad case), and Node.js already installed; it creates the backend venv, installs deps (stripping the unresolvable `emergentintegrations` package), installs Playwright's Chromium, generates `backend/.env` with fresh secrets (the frontend needs no `.env` at all — see "Running locally" below), creates/starts the Mongo container, runs `yarn install`, and runs `yarn build` to produce `frontend/build`. `start.bat` starts Docker Desktop and the Mongo container if needed, then opens **one** window running just the backend — it serves `frontend/build` directly (single origin, single port, see "Production hosting" below), which is what an actual tunnel/LAN visitor needs; it does not start a separate CRA dev server. For active frontend development (hot reload), run `yarn start` by hand instead — see "Running locally" below — then rerun `setup.bat` (or just `yarn build` + restart the backend) to pick the changes up for `start.bat` again. Three batch gotchas worth remembering if editing these: (1) parentheses inside `echo` text break cmd.exe's parser when that `echo` sits inside a parenthesized `if (...)` block — rephrase without parens or move the text outside the block; (2) don't `goto` a label defined inside a parenthesized block (used for the "wait for Docker to be ready" retry loop) — put the label and loop at the top level instead; (3) prefer `corepack yarn ...` over `corepack enable` + `yarn ...` — `enable` writes shim files next to `node.exe` and fails with `EPERM` without admin rights when Node is under `Program Files`, while invoking the pinned version straight through `corepack yarn` needs no shims at all. All three are already handled correctly in both scripts as they stand.
...
5. **Frontend**: no `yarn` binary present, but `package.json` pins `packageManager: yarn@1.22.22` — invoking `corepack yarn ...` (not `corepack enable` + `yarn ...`) fetches and runs that exact version without installing anything.
   ```bash
   cd frontend
   corepack yarn install
   corepack yarn start   # craco start, dev server on :3000
   ```
   No `.env` needed. The frontend always calls a relative `/api/...` path (`src/lib/api.js`'s `API_BASE = '/api'`, hardcoded — no backend-URL environment variable exists anywhere in this codebase); `package.json`'s `"proxy": "http://localhost:8001"` field makes CRA's dev server forward those calls to the backend transparently. The backend still needs to be started separately (`yarn start` only launches the frontend), just nothing needs configuring to connect them.
...
This app is being moved off emergent.sh onto self-hosting (local network / Cloudflare Tunnel to a custom domain), independent of anything emergent-specific above. The frontend and backend are two separate *processes* (`yarn start` on :3000, `uvicorn` on :8001) but always one logical *origin* — the frontend has no concept of a separate backend URL at all:

- `src/lib/api.js` hardcodes `API_BASE = '/api'` — always relative, no environment variable, no way to misconfigure it. There used to be a `REACT_APP_BACKEND_URL` env var here (removed) that got baked into the JS bundle as an absolute URL at build time; it worked on whichever machine built it but broke for every other visitor once this app left `localhost` (a real incident — a Cloudflare-tunneled deploy kept trying to reach each *visitor's own* `localhost:8001`, since that's what "localhost" means from inside a browser tab, wherever it's running). Modeled after how `dc-entry-automation` (a sibling project, `D:\code_files\dc\dc image recognition v3.0.1\dc-entry-automation`) already does this correctly — same architecture, and worth checking if a similar problem ever resurfaces here.
- In dev (`yarn start`, :3000), `package.json`'s `"proxy": "http://localhost:8001"` field makes CRA's built-in dev-server proxy forward any non-HTML request (i.e. anything not a page navigation — which covers all `axios`/`fetch` calls) to the backend transparently. Nothing to configure, nothing to remember to set.
- In production, `server.py` mounts `frontend/build/static` and adds a catch-all `GET /{full_path:path}` route (registered *after* `app.include_router(api_router)`, so `/api/*` always matches first) that serves real static files when they exist and falls back to `index.html` otherwise — required for CRA client-side routes like `/search`/`/history` to work on direct navigation instead of 404ing. This route only activates if `frontend/build/` exists, so plain `uvicorn` + no build present (i.e. normal local dev) is unaffected. Since the frontend calls relative `/api` paths, this same-origin serving is what makes them resolve correctly with zero configuration.
- Rebuild-and-restart flow when you change frontend code: `cd frontend && corepack yarn build`, then restart the backend — the build isn't hot-reloaded, it's a static snapshot served until you rebuild.
...
### Production build
```bash
cd frontend && corepack yarn build   # craco build
```
