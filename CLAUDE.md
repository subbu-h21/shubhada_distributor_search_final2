...
1. **MongoDB** — nothing installed locally; used Docker instead (Docker Desktop present but not auto-started):
   ...
   ```
2. **Backend venv** — the pinned `requirements.txt` needs real wheels; system Python was 3.14 (too new, risks missing wheels for pinned versions), so a **3.11** venv was used instead. `requirements.txt` also lists `emergentintegrations==0.2.0`, a private package only resolvable from emergent.sh's internal index — it is **not imported anywhere in the code**, so it's safe to strip before installing:
   ```bash
   cd backend
   py -3.11 -m venv .venv
